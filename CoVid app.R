...
ui <- navbarPage(
    ...
    tabPanel("Covid counts over time per state",
              sidebarLayout(
                  sidebarPanel(
                      pickerInput("state","Please select one or more state(s):", choices=unique(states_longer['State']),selected = 'South Dakota', options = list(`actions-box` = TRUE),multiple = T)
                  ),
                  mainPanel(
                      plotOutput(outputId = 'q', height = "900px")
                  )
              )
    )
...
server <- function(input, output, session) {
    ...
    output$p <- renderPlotly({
        plot_ly(text = ~paste(US_counties$Combined_Key,'infection count:',US_counties[,input$date])) %>%
            ...
            layout(
                title = "CoVid-19 by U.S. counties"
            ) %>%
            layout(
                geo = g
            )
    ...
    output$q <- renderPlot({
        ...
        geom_line() +
            ylab("CoVid-19 infections per 100,000 population") + 
            xlab("Date")+       
